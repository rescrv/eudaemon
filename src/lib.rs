...
pub use s::eval::{
    builtin_append, builtin_atom_p, builtin_cons, builtin_empty_p, builtin_eq_p, builtin_first,
    builtin_length, builtin_list, builtin_list_p, builtin_nth, builtin_null_p, builtin_rest, eval,
    register_builtins, Env, SExprFn,
};
...
pub use s::markdown::curation::{
    ...
    wrap_in_details, ExtractResult, ExtractStrategy, LinkDefinition, LinkInfo, SkeletonSummary,
    TagInfo,
};
pub use s::markdown::invariants::{
    all_pass, assert_invariant, validate_all, Invariant, ValidationResult, Violation,
};
...
pub use s::nodeid::{
    annotate_document, get_by_content, get_by_path, get_context, get_node, get_parent,
    get_siblings, to_annotated_sexpr, AnnotatedNode, ContentId, NodeId, PathId,
};
pub use s::repl::{register_markdown_builtins, Repl};
pub use s::selector::{
    query, query_one, select, AttributePredicate, Combinator, CompareOp, Selector, SelectorPart,
    SimpleSelector,
};
