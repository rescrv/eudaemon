...
fn build_options() -> Options {
    ...
    opts.optflag("i", "interactive", "Prompt before overwrite (not implemented)");
    ...
    opts.optflag("s", "symbolic-link", "Create symbolic links instead of copying");
    ...
    opts.optflag("P", "no-dereference", "Never follow symlinks (default with -R)");
    opts
}
    ...
    FS: Filesystem,
{
    ...
    let recursive = matches.opt_present("R") || matches.opt_present("r") || matches.opt_present("a");
    ...
    if config.link && config.symlink {
        env.stderr.write_line("cp: the -l and -s options may not be specified together")?;
        return Ok(ExitCode::from(1));
    }
    ...
    let target_is_dir = target_stat.as_ref().map(|e| e.file_type == FileType::Directory).unwrap_or(false);
    ...
    if target_has_slash && !target_is_dir {
        if target_exists {
            env.stderr.write_line(&format!("cp: {}: Not a directory", target))?;
        } else {
            env.stderr.write_line(&format!("cp: {}: No such file or directory", target))?;
        }
        return Ok(ExitCode::from(1));
    }
    ...
    for source in sources {
        let result = if target_is_dir || (sources.len() == 1 && !target_exists && is_source_dir(env, source, &config)) {
    ...
    FS: Filesystem,
{
    ...
    stat.map(|e| e.file_type == FileType::Directory).unwrap_or(false)
}
    ...
    FS: Filesystem,
{
    ...
    if let Ok(target_entry) = env.fs.lstat(target) {
        if source_entry.dev == target_entry.dev && source_entry.ino == target_entry.ino {
            return Err(format!("{} and {} are identical (not copied)", source, target));
        }
    }
    ...
    if target_exists {
        if config.no_clobber {
            if config.verbose {
                env.stdout.write_line(&format!("{} not overwritten", target)).ok();
            }
            return Ok(());
        }
        if config.force {
            env.fs.unlink(target).map_err(|e| format!("{}: {}", target, io_error_message(&e)))?;
        }
    }
    ...
    if config.verbose {
        env.stdout.write_line(&format!("{} -> {}", source, target)).ok();
    }
    ...
    FS: Filesystem,
{
    ...
    queue.push_back((source.trim_end_matches('/').to_string(), target.trim_end_matches('/').to_string()));
        ...
        match entry.file_type {
            FileType::Directory => {
                ...
                if dst_exists {
                    ...
                    let dst_entry = env.fs.lstat(&dst_path)
                    ...
                    env.fs.mkdir(&dst_path)
                        .map_err(|e| format!("{}: {}", dst_path, io_error_message(&e)))?;
                    
                    if config.verbose {
                        env.stdout.write_line(&format!("{} -> {}", src_path, dst_path)).ok();
                    }
                }

                // Queue children
                let children = env.fs.read_dir(&src_path)
                    .map_err(|e| format!("{}: {}", src_path, io_error_message(&e)))?;
            ...
            FileType::Symlink => {
                ...
                let link_target = env.fs.readlink(&src_path)
                    .map_err(|e| format!("{}: {}", src_path, io_error_message(&e)))?;
                ...
                if dst_exists {
                    if config.no_clobber {
                        if config.verbose {
                            env.stdout.write_line(&format!("{} not overwritten", dst_path)).ok();
                        }
                        continue;
                    }
                    if config.force {
                        env.fs.unlink(&dst_path)
                            .map_err(|e| format!("{}: {}", dst_path, io_error_message(&e)))?;
                    } else {
                ...
                env.fs.symlink(&link_target, &dst_path)
                    .map_err(|e| format!("{}: {}", dst_path, io_error_message(&e)))?;
    ...
    FS: Filesystem,
{
    if config.link {
        // Create hard link
        env.fs.link(source, target)
            .map_err(|e| format!("{}: {}", target, io_error_message(&e)))
    } else if config.symlink {
        // Create symbolic link
        env.fs.symlink(source, target)
            .map_err(|e| format!("{}: {}", target, io_error_message(&e)))
    } else {
        // Actually copy the file contents
        // First check if source is a symlink and we should copy the link itself
        let source_lstat = env.fs.lstat(source)
            .map_err(|e| format!("{}: {}", source, io_error_message(&e)))?;
        
        if source_lstat.file_type == FileType::Symlink && !config.follow_all_symlinks && !config.follow_cli_symlinks {
            // Copy symlink
            let link_target = env.fs.readlink(source)
                .map_err(|e| format!("{}: {}", source, io_error_message(&e)))?;
            env.fs.symlink(&link_target, target)
                .map_err(|e| format!("{}: {}", target, io_error_message(&e)))
        } else {
            // Copy file contents
            let contents = env.fs.read_to_string(source)
                .map_err(|e| format!("{}: {}", source, io_error_message(&e)))?;
            env.fs.write_string(target, &contents)
                .map_err(|e| format!("{}: {}", target, io_error_message(&e)))
        }
    }
}
