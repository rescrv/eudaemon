...
**nl** \[**−p**\] \[**−b ***type*\] \[**−d ***delim*\] \[**−f ***type*\]
\[**−h ***type*\] \[**−i ***incr*\] \[**−l ***num*\] \[**−n ***format*\]
\[**−s ***sep*\] \[**−v ***startnum*\] \[**−w ***width*\] \[*file*\]

**DESCRIPTION**

The **nl** utility reads lines from the named *file*, applies a
configurable line numbering filter operation, and writes the result to
the standard output. If *file* is a single dash (‘**−**’) or absent,
...
directives, the text being read is assumed to consist of a single
logical page body.
...
Specify the logical page body lines to be numbered. Recognized *type*
arguments are:
...
Number only those lines that contain the basic regular expression
specified by *expr*.
...
the second character remains unchanged. The default *delim* characters
are “**\\**”.
...
default *incr* value is 1.

**−l** *num*

If numbering of all lines is specified for the current logical section
using the corresponding **−b a**, **−f a** or **−h a** option, specify
...
The default *num* value is 1.

**−n** *format*

Specify the line numbering output format. Recognized *format* arguments
are:
...
Specify the initial value used to number logical page lines; see also
the description of the **−p** option. The default *startnum* value is 1.

**−w** *width*

Specify the number of characters to be occupied by the line number; in
case the *width* is insufficient to hold the line number, it will be
truncated to its *width* least significant digits. The default *width*
is 6.
...
Number lines matching regular expression for an *i* followed by either
*m* or *n*
...
Input lines are limited to LINE_MAX (2048) bytes in length. GNU July 24,
2022 *NL*(1)
