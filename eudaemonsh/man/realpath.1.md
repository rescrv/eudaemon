...
in *path*. If *path* is absent, the current working directory (‘*.*’) is
assumed.

If **−q** is specified, warnings will not be printed when *realpath*(3)
fails.
...
Show the physical path of the */dev/log* directory silencing warnings if
any:
...
The **realpath** utility first appeared in FreeBSD 4.3. GNU June 21,
2011 *REALPATH*(1)
