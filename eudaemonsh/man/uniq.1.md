...
If *input_file* is a single dash (‘**−**’) or absent, the standard input
is read. If *output_file* is absent, standard output is used for output.
...
Output a single copy of each line that is repeated in the input. Ignored
if **−D** is also specified.

**−D**, **−-all-repeated** \[*septype*\]

Output all lines that are repeated (like **−d**, but each copy of the
repeated line is written). The optional *septype* argument controls how
to separate groups of repeated lines in the output; it must be one of
...
A field is a string of non-blank characters separated from adjacent
fields by blanks. Field numbers are one based, i.e., the first field is
field one.
...
option, the first *chars* characters after the first *num* fields will
be ignored. Character numbers are one based, i.e., the first character
is character one.
...
The historic **+***number* and **−***number* options have been
...
A **uniq** command appeared in Version 3 AT&T UNIX. GNU December 9, 2024
*UNIQ*(1)
