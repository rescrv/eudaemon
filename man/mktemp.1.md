...
**mktemp** \[**−d**\] \[**−p ***tmpdir*\] \[**−q**\] \[**−t ***prefix*\]
\[**−u**\] *template ...* **  
mktemp** \[**−d**\] \[**−p ***tmpdir*\] \[**−q**\] \[**−u**\] **−t**
*prefix*
...
*/tmp/temp.XXXXXXXXXX*. The trailing ‘Xs’ are replaced with the current
...
six ‘Xs’ will result in **mktemp** selecting 1 of 56800235584 (62 \*\*
6) possible file names.

If **mktemp** can successfully generate a unique file name, the file is
created with mode 0600 (unless the **−u** flag is given) and the
filename is printed to standard output.

If the **−t** *prefix* option is given, **mktemp** will generate a
template string based on the *prefix* and the TMPDIR environment
variable if set. If the **−p** option is set, then the given *tmpdir*
will be used if the TMPDIR environment variable is not set. Finally,
*/tmp* will be used if neither TMPDIR or **−p** are set and used. Care
should be taken to ensure that it is appropriate to use an environment
variable potentially supplied by the user.

If no arguments are passed or if only the **−d** flag is passed
**mktemp** behaves as if **−t tmp** was supplied.
...
interpreted relative to the path specified as *tmpdir*. If *tmpdir* is
either empty or omitted, then the TMPDIR environment variable will be
used.
...
Generate a template (using the supplied *prefix* and TMPDIR if set) to
...
The directory in which to store temporary files. Refer to *environ*(7).
...
The following *sh*(1) fragment illustrates a simple use of **mktemp**
where the script should quit if it cannot get a safe temporary file.
...
in FreeBSD 2.2.7. This man page is taken from OpenBSD. GNU September 27,
2025 *MKTEMP*(1)
